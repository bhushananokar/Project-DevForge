...
export default function NotebookPage({ params }: { params: { id: string } }) {
  ...
  async function handleDeleteSource(sourceId: string) {
    ...
    try {
      ...
      setNotebook((nb) =>
        nb ? { ...nb, sources: nb.sources.filter((s) => s.id !== sourceId), source_count: nb.source_count - 1 } : nb
      );
    } finally {
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex flex-col gap-4">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  ...
                  <p className="text-secondary text-xs mt-1">
                    Synthesizes all {notebook.source_count} source{notebook.source_count !== 1 ? "s" : ""} into one detailed coding agent prompt.
                  </p>
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-secondary font-medium">
                  Base prompt <span className="text-secondary/60">(optional — your own requirements or constraints)</span>
                </label>
                <textarea
                  rows={3}
                  value={basePrompt}
            ...
            {activePrompt ? (
              <div className="flex flex-col gap-4">
                ...
                {showHistory && prompts.length > 1 && (
                  <div className="flex flex-col gap-2 mt-2">
                    <h4 className="text-xs font-medium text-secondary uppercase tracking-wide">History</h4>
              ...
              <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                ...
                <p className="text-secondary text-sm max-w-sm">
                  Add your sources and click Generate to create a detailed coding agent prompt.
                </p>
              </div>
            )}
          </div>
